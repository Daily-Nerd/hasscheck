...
def repairs(
    ...
    """Generate a repairs.py starter for a Home Assistant custom integration."""
    path = path.resolve()
    ...
    if not force:
        warning = check_applicability_gate(config, "repairs")
        if warning:
            console.print(f"[yellow]Warning:[/] {warning}")
            raise typer.Exit(code=1)
    ...
    if context.integration_path is not None:
        target = context.integration_path / "repairs.py"
    else:
        target = path / "custom_components" / domain / "repairs.py"

    template_content = load_template("repairs.py.tmpl")
    content = render(template_content)
